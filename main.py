...
# MQTT AT commands
def publish_mqtt_data(data, data_detected):
    timestamp = time.localtime() 
    formatted_time = "{:04}-{:02}-{:02} {:02}:{:02}:{:02}".format(timestamp[0], timestamp[1], timestamp[2], timestamp[3], timestamp[4], timestamp[5])
    json_data = ujson.dumps(data)
    #payload = {data_detected: timestamp}
    #json_detected = ujson.dumps(payload)
    result_length = len(json_data) + len(formatted_time) + len(data_detected)
    result_data = formatted_time + data_detected +json_data
    ...
    at_commands = [
        ...
        f'AT+QMTPUBEX=0,0,0,0,"energy_values",{result_length}',
        result_data,
        'AT+QMTDISC=0'
    ]
    # Send MQTT commands
    ...
    while True: 
        try:
            ...
            if coordinates[0] and coordinates[1]: 
                ...
                if last_lat and last_lon:
                    ...
                    if distance >= Threshold:
                        ...
                        publish_mqtt_data(data, " Movement detected: ")
            ...
            if time.time() - last_publish_time >= settime:
                ...
                publish_mqtt_data(data, " Time exceed: ")
                log_message(f"Energy and GPS data published: {data}")
                last_publish_time = time.time()
                gc.collect()
